...
class ClothesShopApp:
    def __init__(self, window):
        self.window = window
        self.window.title("Clothes Shop")
    ...
    def create_widgets(self):
        title_label = ttk.Label(self.window, text="Clothes Shop", foreground="#E2E2B6", font=('ADLaM Display', 35, 'bold'),background="#fdf5df")
    ...
    def save_product_data(self):
        try:
            conn = sqlite3.connect(DATABASE_FILE)
            cursor = conn.cursor()
            cursor.execute('CREATE TABLE IF NOT EXISTS product_Clothes (name TEXT, brand TEXT, price REAL, image_name TEXT)')
            cursor.execute('DELETE FROM product_Clothes')
            for spinbox in self.spinboxes:
                details = spinbox.product_details
                cursor.execute('''
                    INSERT INTO product_Clothes (name, brand, price, image_name)
                    VALUES (?, ?, ?, ?)
    ...
    def load_product_data(self):
        try:
            ...
            cursor = conn.cursor()
            cursor.execute('CREATE TABLE IF NOT EXISTS product_Clothes (name TEXT, brand TEXT, price REAL, image_name TEXT)')
            cursor.execute('SELECT name, brand, price, image_name FROM product_Clothes')
