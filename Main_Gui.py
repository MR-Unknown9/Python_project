...
class MainPage:
    def __init__(self, window):
        ...
        self.window.iconbitmap(r"D:\python pr\My git_hub_proj\Python_project\images\icoPerfum.ico")
        self.create_styles()
        self.create_widgets()

    def create_styles(self):
        style = ttk.Style()
        style.configure("info.TLabelframe",lightcolor="#fdf5df",darkcolor="#fdf5df", foreground="#fdf5df", bordercolor="#fdf5df",background="#fdf5df")
    ...
    def create_widgets(self):
        # Frame for buttons
        Info_frame = ttk.Labelframe(self.window, style='info.TLabelframe', text='Nave Bar', width=200)
    ...
    def create_buttons(self, frame):
        ...
        for i, (text, style, *command) in enumerate(buttons):
            button = ttk.Button(frame, text=text, style=style, command=command[0] if command else None)
    ...
    def create_labels(self):
        ttk.Label(self.window, text="Online Shopping", font="roboto 35 bold", foreground="#6EACDA",background="#fdf5df").place(x=500, y=100)
        ttk.Label(self.window, text="""Welcome to our online store!
We are delighted to have you join our family.
If you have any questions or need assistance,
our support team is here to help.
Happy shopping!""", font="roboto 10 bold", foreground="#E2E2B6",background="#fdf5df").place(x=500, y=170)

    def create_images(self):
        # Create a canvas to hold the image with a background color
        canvas1 = tk.Canvas(self.window, width=60, height=60, bg="#fdf5df", bd=0, highlightthickness=0)
        ...
        self.window.image_tk1 = image_tk1  # Keep a reference to avoid garbage collection

        # Repeat for additional images
        canvas2 = tk.Canvas(self.window, width=340, height=340, bg="#fdf5df", bd=0, highlightthickness=0)
        ...
        canvas3 = tk.Canvas(self.window, width=300, height=300, bg="#fdf5df", bd=0, highlightthickness=0)
